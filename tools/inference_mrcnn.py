...
def forwardpass( dense_img_bson, weights_filepath, cfg_filepath, device="cpu" ):
...
if __name__ == "__main__":
    ...
    io = larcv.IOManager(larcv.IOManager.kBOTH,"supera",larcv.IOManager.kTickBackward)
    io.set_out_file( "out_test.root" )
    io.add_in_file( supera_file )
    io.specify_data_read( larcv.kProductImage2D, "wire" )
    io.reverse_all_products()
    io.initialize()
    weight_dir = "/home/twongjirad/working/larbys/ubdl/ublarcvserver/networks/mask-rcnn.pytorch/weights/"
    #weights = [ weight_dir+"/mcc8_mrcnn_plane{}.pth".format(x) for x in range(3) ]
    weights = [ weight_dir+"/{}_plane.pth".format(x) for x in ("u","v","y") ]
    ...
    config_dir = "/home/twongjirad/working/larbys/ubdl/ublarcvserver/networks/mask-rcnn.pytorch/configs/baselines/"
    configs = [ config_dir+"/mills_config_{}_full_image.yaml".format(x) for x in range(3) ]
    DEVICE="cuda:0"
    ...
    for ientry in range(nentries):
        ...
        ev_mask = io.get_data( larcv.kProductClusterMask, "smaskrcnn" )        
        for p in [0,1,2]:
            ...
            out_v = forwardpass( bson, weights[p], configs[p], device=DEVICE )
            if out_v is not None:
                print("plane {} returned with {} outputs".format(p,len(out_v)))
            else:
                print("plane {} returned None".format(p))

            mask_v = std.vector("larcv::ClusterMask")()
                
            for bson_mask in out_v:
                ret_run = c_int()
                ret_subrun = c_int()
                ret_event  = c_int()
                ret_id     = c_int()
                mask = larcv.json.clustermask_from_pybytes( bson_mask, ret_run, ret_subrun, ret_event, ret_id );
                print(ret_run,ret_subrun,ret_event,ret_id)
                mask_v.push_back(mask)                
            ev_mask.append(mask_v)
        print("[save entry]")
        io.save_entry()

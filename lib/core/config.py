...
def make_default_config():
    ...
    C.TRAIN.DATASETS = ('particle_physics_train')
    ...
    C.TRAIN.BATCH_SIZE_PER_IM = 64
    
    # Fraction of minibatch that is labeled foreground (i.e. class > 0)
    C.TRAIN.FG_FRACTION = 0.25
    
    # Overlap threshold for a ROI to be considered foreground (if >= FG_THRESH)
    C.TRAIN.FG_THRESH = 0.5
    
    # Overlap threshold for a ROI to be considered background (class = 0 if
    # overlap in [LO, HI))
    C.TRAIN.BG_THRESH_HI = 0.5
    C.TRAIN.BG_THRESH_LO = 0.0
    
    # Use horizontally-flipped images during training?
    C.TRAIN.USE_FLIPPED = False
    ...
    # TRAIN.DATASETS
    C.TRAIN.PROPOSAL_FILES = ()
    
    # Snapshot (model checkpoint) period
    # Divide by NUM_GPUS to determine actual period (e.g., 20000/8 => 2500 iters)
    # to allow for linear training schedule scaling
    C.TRAIN.SNAPSHOT_ITERS = 10000
    
    # Normalize the targets (subtract empirical mean, divide by empirical stddev)
    C.TRAIN.BBOX_NORMALIZE_TARGETS = True
    # Deprecated (inside weights)
    C.TRAIN.BBOX_INSIDE_WEIGHTS = (1.0, 1.0, 1.0, 1.0)
    # Normalize the targets using "precomputed" (or made up) means and stdevs
    # (BBOX_NORMALIZE_TARGETS must also be True) (legacy)
    C.TRAIN.BBOX_NORMALIZE_TARGETS_PRECOMPUTED = False
    C.TRAIN.BBOX_NORMALIZE_MEANS = (0.0, 0.0, 0.0, 0.0)
    C.TRAIN.BBOX_NORMALIZE_STDS = (0.1, 0.1, 0.2, 0.2)
    
    # Make minibatches from images that have similar aspect ratios (i.e. both
    # tall and thin or both short and wide)
    # This feature is critical for saving memory (and makes training slightly
    # faster)
    C.TRAIN.ASPECT_GROUPING = True
    
    # Crop images that have too small or too large aspect ratio
    C.TRAIN.ASPECT_CROPPING = False
    C.TRAIN.ASPECT_HI = 2
    C.TRAIN.ASPECT_LO = 0.5
    
    # For printing Out images in the midst of training
    C.TRAIN.MAKE_IMAGES = False
    # For culling Entrys in ROIDB with GT max IOU less than this
    C.TRAIN.GT_IOU_THRESH = 0.0
    ...
    # cases)
    # C.TRAIN.RPN_POST_NMS_TOP_N = 2000 # used in 2 PLANE
    C.TRAIN.RPN_POST_NMS_TOP_N = 1000 # trying in 1 PLANE
    
    
    # Remove RPN anchors that go outside the image by RPN_STRADDLE_THRESH pixels
    # Set to -1 or a large value, e.g. 100000, to disable pruning anchors
    C.TRAIN.RPN_STRADDLE_THRESH = 0
    
    # Proposal height and width both need to be greater than RPN_MIN_SIZE
    # (at orig image scale; not scale used during training or inference)
    C.TRAIN.RPN_MIN_SIZE = 0
    ...
    C.TEST.FORCE_JSON_DATASET_EVAL = False
    ...
    C.TEST.PRECOMPUTED_PROPOSALS = True
    ...
    C.TEST.BBOX_AUG.SCORE_HEUR = 'UNION'
    
    # Heuristic used to combine predicted box coordinates
    #   Valid options: ('ID', 'AVG', 'UNION')
    C.TEST.BBOX_AUG.COORD_HEUR = 'UNION'
    
    # Horizontal flip at the original scale (id transform)
    C.TEST.BBOX_AUG.H_FLIP = False
    
    # Each scale is the pixel size of an image's shortest side
    C.TEST.BBOX_AUG.SCALES = ()
    
    # Max pixel size of the longer side
    C.TEST.BBOX_AUG.MAX_SIZE = 4000
    
    # Horizontal flip at each scale
    C.TEST.BBOX_AUG.SCALE_H_FLIP = False
    
    # Apply scaling based on object size
    C.TEST.BBOX_AUG.SCALE_SIZE_DEP = False
    C.TEST.BBOX_AUG.AREA_TH_LO = 50**2
    C.TEST.BBOX_AUG.AREA_TH_HI = 180**2
    
    # Each aspect ratio is relative to image width
    C.TEST.BBOX_AUG.ASPECT_RATIOS = ()
    
    # Horizontal flip at each aspect ratio
    C.TEST.BBOX_AUG.ASPECT_RATIO_H_FLIP = False
    
    # ---------------------------------------------------------------------------- #
    # Test-time augmentations for mask detection
    # See configs/test_time_aug/e2e_mask_rcnn_R-50-FPN_2x.yaml for an example
    # ---------------------------------------------------------------------------- #
    C.TEST.MASK_AUG = AttrDict()
    
    # Enable test-time augmentation for instance mask detection if True
    C.TEST.MASK_AUG.ENABLED = False
    ...
    C.TEST.MASK_AUG.HEUR = 'SOFT_AVG'
    
    # Horizontal flip at the original scale (id transform)
    C.TEST.MASK_AUG.H_FLIP = False
    
    # Each scale is the pixel size of an image's shortest side
    C.TEST.MASK_AUG.SCALES = ()
    
    # Max pixel size of the longer side
    C.TEST.MASK_AUG.MAX_SIZE = 4000
    
    # Horizontal flip at each scale
    C.TEST.MASK_AUG.SCALE_H_FLIP = False
    
    # Apply scaling based on object size
    C.TEST.MASK_AUG.SCALE_SIZE_DEP = False
    C.TEST.MASK_AUG.AREA_TH = 180**2
    
    # Each aspect ratio is relative to image width
    C.TEST.MASK_AUG.ASPECT_RATIOS = ()
    
    # Horizontal flip at each aspect ratio
    C.TEST.MASK_AUG.ASPECT_RATIO_H_FLIP = False
    
    # ---------------------------------------------------------------------------- #
    # Test-augmentations for keypoints detection
    # configs/test_time_aug/keypoint_rcnn_R-50-FPN_1x.yaml
    # ---------------------------------------------------------------------------- #
    C.TEST.KPS_AUG = AttrDict()
    
    # Enable test-time augmentation for keypoint detection if True
    C.TEST.KPS_AUG.ENABLED = False
    
    # Heuristic used to combine keypoint predictions
    #   Valid options: ('HM_AVG', 'HM_MAX')
    C.TEST.KPS_AUG.HEUR = 'HM_AVG'
    
    # Horizontal flip at the original scale (id transform)
    C.TEST.KPS_AUG.H_FLIP = False
    
    # Each scale is the pixel size of an image's shortest side
    C.TEST.KPS_AUG.SCALES = ()
    
    # Max pixel size of the longer side
    C.TEST.KPS_AUG.MAX_SIZE = 4000
    
    # Horizontal flip at each scale
    C.TEST.KPS_AUG.SCALE_H_FLIP = False
    
    # Apply scaling based on object size
    C.TEST.KPS_AUG.SCALE_SIZE_DEP = False
    C.TEST.KPS_AUG.AREA_TH = 180**2
    
    # Eeach aspect ratio is realtive to image width
    C.TEST.KPS_AUG.ASPECT_RATIOS = ()
    
    # Horizontal flip at each aspect ratio
    C.TEST.KPS_AUG.ASPECT_RATIO_H_FLIP = False
    
    # ---------------------------------------------------------------------------- #
    # Soft NMS
    # ---------------------------------------------------------------------------- #
    C.TEST.SOFT_NMS = AttrDict()
    
    # Use soft NMS instead of standard NMS if set to True
    C.TEST.SOFT_NMS.ENABLED = False
    # See soft NMS paper for definition of these options
    C.TEST.SOFT_NMS.METHOD = 'linear'
    C.TEST.SOFT_NMS.SIGMA = 0.5
    # For the soft NMS overlap threshold, we simply use TEST.NMS
    
    # ---------------------------------------------------------------------------- #
    # Bounding box voting (from the Multi-Region CNN paper)
    # ---------------------------------------------------------------------------- #
    C.TEST.BBOX_VOTE = AttrDict()
    
    # Use box voting if set to True
    C.TEST.BBOX_VOTE.ENABLED = False
    ...
    C.TEST.BBOX_VOTE.SCORING_METHOD = 'ID'
    
    # Hyperparameter used by the scoring method (it has different meanings for
    # different methods)
    C.TEST.BBOX_VOTE.SCORING_METHOD_BETA = 1.0
    ...
    C.MODEL.BBOX_REG_WEIGHTS = (10., 10., 5., 5.)
    ...
    C.MODEL.SHARE_RES5 = False
    
    # Whether to load imagenet pretrained weights
    # If True, path to the weight file must be specified.
    # See: C.RESNETS.IMAGENET_PRETRAINED_WEIGHTS
    C.MODEL.LOAD_IMAGENET_PRETRAINED_WEIGHTS = True
    
    # ---------------------------------------------------------------------------- #
    # Unsupervise Pose
    # ---------------------------------------------------------------------------- #
    
    C.MODEL.UNSUPERVISED_POSE = False
    
    
    # ---------------------------------------------------------------------------- #
    # RetinaNet options
    # ---------------------------------------------------------------------------- #
    C.RETINANET = AttrDict()
    
    # RetinaNet is used (instead of Fast/er/Mask R-CNN/R-FCN/RPN) if True
    C.RETINANET.RETINANET_ON = False
    
    # Anchor aspect ratios to use
    C.RETINANET.ASPECT_RATIOS = (0.5, 1.0, 2.0)
    
    # Anchor scales per octave
    C.RETINANET.SCALES_PER_OCTAVE = 3
    ...
    C.RETINANET.ANCHOR_SCALE = 4
    
    # Convolutions to use in the cls and bbox tower
    # NOTE: this doesn't include the last conv for logits
    C.RETINANET.NUM_CONVS = 4
    
    # Weight for bbox_regression loss
    C.RETINANET.BBOX_REG_WEIGHT = 1.0
    
    # Smooth L1 loss beta for bbox regression
    C.RETINANET.BBOX_REG_BETA = 0.11
    
    # During inference, #locs to select based on cls score before NMS is performed
    # per FPN level
    C.RETINANET.PRE_NMS_TOP_N = 1000
    
    # IoU overlap ratio for labeling an anchor as positive
    # Anchors with >= iou overlap are labeled positive
    C.RETINANET.POSITIVE_OVERLAP = 0.5
    
    # IoU overlap ratio for labeling an anchor as negative
    # Anchors with < iou overlap are labeled negative
    C.RETINANET.NEGATIVE_OVERLAP = 0.4
    
    # Focal loss parameter: alpha
    C.RETINANET.LOSS_ALPHA = 0.25
    
    # Focal loss parameter: gamma
    C.RETINANET.LOSS_GAMMA = 2.0
    
    # Prior prob for the positives at the beginning of training. This is used to set
    # the bias init for the logits layer
    C.RETINANET.PRIOR_PROB = 0.01
    
    # Whether classification and bbox branch tower should be shared or not
    C.RETINANET.SHARE_CLS_BBOX_TOWER = False
    
    # Use class specific bounding box regression instead of the default class
    # agnostic regression
    C.RETINANET.CLASS_SPECIFIC_BBOX = False
    
    # Whether softmax should be used in classification branch training
    C.RETINANET.SOFTMAX = False
    
    # Inference cls score threshold, anchors with score > INFERENCE_TH are
    # considered for inference
    C.RETINANET.INFERENCE_TH = 0.05
    ...
    C.SOLVER.WEIGHT_DECAY_GN = 0.0

    # Whether to double the learning rate for bias
    C.SOLVER.BIAS_DOUBLE_LR = True
    
    # Whether to have weight decay on bias as well
    C.SOLVER.BIAS_WEIGHT_DECAY = False
    
    # Warm up to SOLVER.BASE_LR over this number of SGD iterations
    C.SOLVER.WARM_UP_ITERS = 500
    
    # Start the warm up from SOLVER.BASE_LR * SOLVER.WARM_UP_FACTOR
    C.SOLVER.WARM_UP_FACTOR = 1.0 / 3.0
    
    # WARM_UP_METHOD can be either 'constant' or 'linear' (i.e., gradual)
    C.SOLVER.WARM_UP_METHOD = 'linear'
    ...
    C.SOLVER.SCALE_MOMENTUM_THRESHOLD = 1.1
    
    # Suppress logging of changes to LR unless the relative change exceeds this
    # threshold (prevents linear warm up from spamming the training log)
    C.SOLVER.LOG_LR_CHANGE_THRESHOLD = 1.1


    # ---------------------------------------------------------------------------- #
    # Fast R-CNN options
    # ---------------------------------------------------------------------------- #
    C.FAST_RCNN = AttrDict()
    ...
    C.RPN.OUT_DIM_AS_IN_DIM = True
    
    # Output dim of conv2d. Ignored if `C.RPN.OUT_DIM_AS_IN_DIM` is True.
    ...
    C.RPN.SIZES = (64, 128, 256, 512)

    # Stride of the feature map that RPN is attached
    C.RPN.STRIDE = 16
    
    # RPN anchor aspect ratios
    C.RPN.ASPECT_RATIOS = (0.5, 1, 2)
    ...
    C.FPN.MULTILEVEL_ROIS = False
    # Hyperparameters for the RoI-to-FPN level mapping heuristic
    C.FPN.ROI_CANONICAL_SCALE = 224  # s0
    C.FPN.ROI_CANONICAL_LEVEL = 4  # k0: where s0 maps to
    # Coarsest level of the FPN pyramid
    C.FPN.ROI_MAX_LEVEL = 5
    # Finest level of the FPN pyramid
    C.FPN.ROI_MIN_LEVEL = 2
    
    # Use FPN for RPN if True
    C.FPN.MULTILEVEL_RPN = False
    # Coarsest level of the FPN pyramid
    C.FPN.RPN_MAX_LEVEL = 6
    # Finest level of the FPN pyramid
    C.FPN.RPN_MIN_LEVEL = 2
    # FPN RPN anchor aspect ratios
    C.FPN.RPN_ASPECT_RATIOS = (0.5, 1, 2)
    ...
    C.RESNETS.NUM_GROUPS = 1
    
    # Baseline width of each group
    C.RESNETS.WIDTH_PER_GROUP = 64
    
    # Place the stride 2 conv on the 1x1 filter
    # Use True only for the original MSRA ResNet; use False for C2 and Torch models
    C.RESNETS.STRIDE_1X1 = True
    
    # Residual transformation function
    C.RESNETS.TRANS_FUNC = 'bottleneck_transformation'
    # ResNet's stem function (conv1 and pool1)
    C.RESNETS.STEM_FUNC = 'basic_bn_stem'
    # ResNet's shortcut function
    C.RESNETS.SHORTCUT_FUNC = 'basic_bn_shortcut'
    
    # Apply dilation in stage "res5"
    C.RESNETS.RES5_DILATION = 1
    ...
    # be fixed.
    C.RESNETS.FREEZE_AT = 2
    
    # Path to pretrained resnet weights on ImageNet.
    # If start with '/', then it is treated as a absolute path.
    # Otherwise, treat as a relative path to C.ROOT_DIR
    C.RESNETS.IMAGENET_PRETRAINED_WEIGHTS = ''
    
    # Use GroupNorm instead of BatchNorm
    C.RESNETS.USE_GN = False
    

    # ---------------------------------------------------------------------------- #
    # GroupNorm options
    # ---------------------------------------------------------------------------- #
    C.GROUP_NORM = AttrDict()
    # Number of dimensions per group in GroupNorm (-1 if using NUM_GROUPS)
    C.GROUP_NORM.DIM_PER_GP = -1
    # Number of groups in GroupNorm (-1 if using DIM_PER_GP)
    C.GROUP_NORM.NUM_GROUPS = 32
    # GroupNorm's small constant in the denominator
    C.GROUP_NORM.EPSILON = 1e-5
    ...
    C.BBOX_XFORM_CLIP = np.log(1000. / 16.)
    ...
    C.PIXEL_MEANS = np.array([[[0.0, 0.0, 0.0]]])
    ...
    # Output basedir
    C.OUTPUT_DIR = '/media/disk1/jmills/Outputs'
    ...
    C.EXPECTED_RESULTS_RTOL = 0.1
    C.EXPECTED_RESULTS_ATOL = 0.005
    # Set to send email in case of an EXPECTED_RESULTS failure
    C.EXPECTED_RESULTS_EMAIL = ''
    
    # ------------------------------
    # Data directory
    C.DATA_DIR = osp.abspath(osp.join(C.ROOT_DIR, 'data'))
    ...
    C.CROP_RESIZE_WITH_MAX_POOL = True
    ...
    # [Infered value]
    C.PYTORCH_VERSION_LESS_THAN_040 = False

    return C


__C = make_default_config()
